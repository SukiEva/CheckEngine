...
class SqlExecutor:
    ...
    @staticmethod
    def _open_session(datasource: DatasourceLike) -> AbstractContextManager[SessionLike]:
        session_or_cm = datasource.get_session()
        if hasattr(session_or_cm, "__enter__") and hasattr(session_or_cm, "__exit__"):
            return cast(AbstractContextManager[SessionLike], session_or_cm)
        return SqlExecutor._wrap_session_iterator(cast(Iterator[SessionLike], session_or_cm))

    @staticmethod
    @contextmanager
    def _wrap_session_iterator(session_iterator: Iterator[SessionLike]) -> Iterator[SessionLike]:
        yield from session_iterator
